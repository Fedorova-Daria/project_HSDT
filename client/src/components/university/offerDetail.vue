<template>
  ...
  <path d="M11.7 0.7C11.3134 0.3134 10.6866 0.313401 10.3 0.7L4.2 6.8L2.1 4.7C1.7134 4.3134 1.0866 4.3134 0.7 4.7V4.7C0.313401 5.0866 0.3134 5.7134 0.699999 6.1L2.78579 8.18579C3.56683 8.96683 4.83316 8.96683 5.61421 8.18579L11.7 2.1C12.0866 1.7134 12.0866 1.0866 11.7 0.7V0.7Z" fill="white"/>
      ...
      <button  @click="handleDeleteOffer(offerId)" v-if="isOwner" class="h-6 w-6"><svg width="22" height="22" viewBox="0 0 14 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
import UserService from "@/composables/storage";
  ...
  export default {
    ...
    liked() {
       ...
       */
      isOwner() {
        return this.offer.owner && this.offer.owner === this.currentUser.id;
      },
      /**
    ...
    watch: {
      offerId: {
        immediate: true,
        handler(newId) {
          console.log("Получен новый offerId:", newId);
          if (newId) {
  ...
  try {
    ...
    // Проверяем статус вместо обращения к response.data
    if (response.status === 204 || response.status === 200) {
      this.$router.back();
    } else {
      this.$toast.error("Произошла ошибка при удалении.");
      console.warn("Неожиданный ответ при удалении:", response);
    }
    ...
    this.$toast.error("Не удалось удалить проект. Попробуйте позже.");
      ...
      toggleEditing() {
        ...
        if (this.isEditing) {
          ...
          this.editedoffer.title = this.offer.title;
          this.editedoffer.description = this.offer.description;
          // Копирование массивов копированием через spread необходимо для независимости
          this.editedoffer.skills_required = [...(this.offer.skills_required || [])];
          this.editedoffer.status = this.offer.status ?? "";
        }
      },
      /**
      ...
      async saveAllChanges() {
        try {
          ...
          const payload = {
            ...(this.editedoffer.title && { title: this.editedoffer.title }),
            ...(this.editedoffer.description && { description: this.editedoffer.description }),
            ...(this.editedoffer.skills_required && { skills_required: this.editedoffer.skills_required }),
            status: this.editedoffer.status || null,
          };
          ...
          this.fetchofferDetails(this.offer.id);
        } catch (error) {
          console.error("Ошибка при сохранении:", error.response?.data || error);
        }
      },
      /**
     * Загружает данные идеи по её ID, используя метод из `offers.js`
     */
    async fetchofferDetails(id) {
      try {
        const response = await api.get(`/offers/${id}/`);
        this.offer = response.data;
        console.log("Данные проекта:", this.offer);
