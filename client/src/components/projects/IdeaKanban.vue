<template>
 ...
 <!-- Индикатор загрузки -->
    <div v-if="isLoading" class="flex justify-center items-center h-screen">
      ...
      <span class="ml-3 text-lg">Проверка доступа...</span>
    </div>

    <!-- Сообщение об отсутствии доступа -->
    <div v-else-if="!hasAccess" class="flex justify-center items-center h-screen">
      <div class="text-center">
        ...
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Доступ запрещен</h2>
        <p class="text-gray-600 dark:text-gray-400 mb-4">
          У вас нет прав для просмотра этой канбан доски.
        </p>
        <p class="text-sm text-gray-500">
          Доступ имеют только участники команды и пользователи с ролью EX.
        </p>
        <button 
          @click="$router.go(-1)" 
    ...
    <!-- ✅Канбан доска отображается только при наличии доступа -->
    <div v-else class="flex-1 overflow-auto custom-scrollbar-horizontal">
...
export default {
  ...
  data() {
    return {
      ...
      userRole: null,
    currentUserId: null,
    hasAccess: false, // ✅ ДОБАВЛЕНО: флаг доступа
    isLoading: true,  // ✅ ДОБАВЛЕНО: состояние загрузки
    };
  },
  created() {
  // ✅ ДОБАВЛЕНО: инициализация данных пользователя
...
computed: {
  isExpertUser() {
    return this.userRole === 'EX';
  },
  
  canManageBoard() {
    return this.isExpertUser; // EX пользователи могут управлять доской
  }
},
  methods: {
    async checkAccess() {
    try {
      ...
      // Если роль EX - доступ разрешен
      if (this.userRole === 'EX') {
        this.hasAccess = true;
        return;
      }

      // Если не EX, проверяем участие в команде
  ...
  async checkTeamMembership() {
    try {
      // Сначала получаем информацию о доске идеи
      const boardResponse = await api.get(`/kanban/boards/?idea=${this.ideaId}`);
      
      let teamId = null;
      if (Array.isArray(boardResponse.data)) {
        teamId = boardResponse.data[0]?.team;
      } else {
        teamId = boardResponse.data?.team;
      }
      ...
      this.hasAccess = teamMembers.some(member => member.id === this.currentUserId);
  ...
  try {
    // ✅ ИЗМЕНЕНО: используем ideaId вместо projectId
    const response = await api.get(`/kanban/boards/?idea=${this.ideaId}`);
    ...
    if (Array.isArray(response.data)) {
      ...
      if (board) {
        ...
        this.teamId = board.team; // ✅ ДОБАВЛЕНО: сохраняем teamId для проверки доступа
      } else {
        this.columns = [];
        this.boardId = null;
      }
      ...
      this.teamId = response.data.team; // ✅ ДОБАВЛЕНО: сохраняем teamId
    }
