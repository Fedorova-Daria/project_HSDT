...
<div class="mt-auto flex justify-end gap-2">
    <img class="w-6 h-6 ml-55 cursor-pointer" src="/pencil1.svg"/>
...
export default { components: {Header},
  ...
  methods: {
    async loadOwnerName() {
  try {
    // Если поле owner существует в идее, загружаем имя владельца
    if (this.idea.owner) {
  ...
  try {
    ...
    if (this.idea.owner) {
      await this.loadOwnerName(); // Загружаем имя владельца
    }
  } catch (error) {
